    ...
    <div>
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-gray-400">View your earnings and work statistics</p>
        </div>
        <div className="flex items-center gap-4">
          <select
            value={selectedYear}
            onChange={(e) => setSelectedYear(parseInt(e.target.value))}
            className="bg-gray-700 border border-gray-600 text-white px-3 py-2 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
            {years.map(year => (
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-700">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm text-gray-400">Avg Weekly Pay</p>
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">Loading payroll history...</p>
        </div>
      ) : payrollHistory.length > 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full">
              <thead className="bg-gray-700">
                <tr>
                  <th className="text-left py-3 px-4 font-semibold text-gray-300">Period</th>
                  <th className="text-left py-3 px-4 font-semibold text-gray-300">Time</th>
                  <th className="text-right py-3 px-4 font-semibold text-gray-300">Hours</th>
                  <th className="text-right py-3 px-4 font-semibold text-gray-300">Overtime</th>
                  <th className="text-right py-3 px-4 font-semibold text-gray-300">Base Pay</th>
                  <th className="text-right py-3 px-4 font-semibold text-gray-300">Overtime Pay</th>
                  <th className="text-right py-3 px-4 font-semibold text-gray-300">Deductions</th>
                  ...
                  <tr key={entry.id} className="hover:bg-gray-700/50">
                    ...
                    <td className="py-3 px-4">
                      <div className="text-sm">
                        <p className="text-gray-300">In: {formatTime(entry.clock_in_time)}</p>
                        <p className="text-gray-400">Out: {formatTime(entry.clock_out_time)}</p>
                      </div>
                    </td>
        ...
        <div className="text-center py-12">
          ...
          <p className="text-gray-400">No payroll records found for {selectedYear}.</p>
        </div>
      )}
    </div>
  );
